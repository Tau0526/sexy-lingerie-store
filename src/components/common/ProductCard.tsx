...
const ProductCard = ({ data, theme = "light" }: ProductCardProps) => {
  ...
  return (
    <article
      className={cn([
        "group relative flex h-full min-w-0 flex-col bg-transparent pb-1 text-center transition-all duration-500 ease-out",
    ...
    >
      <WishlistButton
        ...
        className="absolute right-3 top-3 z-[3] h-8 w-8"
      />
      <Link
        href={`/shop/product/${data.id}/${data.slug}`}
        className="flex h-full min-w-0 flex-col outline-none focus-visible:ring-2 focus-visible:ring-moonlite-gold focus-visible:ring-offset-2 focus-visible:ring-offset-moonlite-ivory"
      >
        <div
          className={cn([
